...
class ChartInterpreter:
	...
	def __init__(self, result: RenderResult, mode: str, lookup: Dict[str, Any]):
		try:
			self.result = result
			self.visible_objects = result.visible_objects
			print(f"Visible objects: {self.visible_objects}")
			self.mode = mode
			self.lookup = lookup
			
			# Pull the DF out of the plot_data dictionary we just populated
			raw_df = result.plot_data.get('ordered_df')
			
			if raw_df is not None:
				# Use str.strip() to ensure no whitespace ruins the match
				self.ordered_df = raw_df[raw_df['Object'].str.strip().isin(self.visible_objects)].copy()
			else:
				self.ordered_df = None

			# Directly use the edges captured during drawing
			self.drawn_major_edges = result.drawn_major_edges
			print(f"Edges major: {self.drawn_major_edges}")
			self.drawn_minor_edges = result.drawn_minor_edges
			print(f"Edges minor: {self.drawn_minor_edges}")
			
		except Exception as e:
			logging.error(f"Error in ChartInterpreter __init__: {e}")
			raise	
	...
	def generate(self, mode: Optional[str] = None) -> str:
		...
		if self.ordered_df is None or self.ordered_df.empty:
			return f"Data Sync Error: I see {len(self.visible_objects)} objects on the chart, but their data didn't transfer to the interpreter."
		...
		# 2. Compass/Axes
		if self.result.compass_rose_on:
		...
		clusters, cluster_map, clustered_members = build_conjunction_clusters(self.ordered_df, self.drawn_major_edges)
		aspect_texts = []
		for a_orig, b_orig, meta in self.drawn_major_edges:
