...
if st.session_state.get("chart_ready", False):
    ...
    # --- Focus quick-jump (single source of truth is st.session_state.selected_placement)
    # Sync selectbox with current selection (last action wins)
    if st.session_state.selected_placement and st.session_state.selected_placement in ORDERED_OBJECTS_FOCUS:
        st.session_state.focus_select = st.session_state.selected_placement

    chosen = st.sidebar.selectbox(
        "Focus a Placement",
        ORDERED_OBJECTS_FOCUS,             # full list; scrollable by default
        index=(ORDERED_OBJECTS_FOCUS.index(st.session_state.focus_select)
               if st.session_state.focus_select in ORDERED_OBJECTS_FOCUS else 0),
        key="focus_select",
        help="Quick-jump to a single placement. This overrides other visibility toggles while focused."
    )

    # If user changed the selectbox, update canonical selection
    ...
    focus_core = _focus_core_from_df(df, focus_target)
    active_shapes = []
    ...
    if focus_core:
        ...
        # Effective visible set is the focus core
    ...
    if focus_core:
        visible_objects = set(focus_core)
    ...
    for obj in ordered_objects:
        ...
        with btn_col:
            ...
            if st.button(label, key=f"focus_{obj}"):
                ...
                st.rerun()  # re-run so the selectbox can be pre-seeded safely
