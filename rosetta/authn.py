...
def ensure_user_row_linked(current_user_id: str, creds: dict, display_name: str | None = None) -> tuple[bool, str]:
    sb = supa()
    exists = (
        sb.table("users")
        .select("username")
    ...
    if not pw_hash:
        return False, ("This login exists in authenticator, but no password hash was found in creds. "
    ...
    return True, f"Linked '{current_user_id}' to DB users (no password change). Reloading…"