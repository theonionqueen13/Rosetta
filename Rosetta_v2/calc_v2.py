    ...
    def _one_system_rows(sys: str) -> list[dict]:
        ...
        if sys_lc == "placidus":
            ...
            if asc_for_whole is None:
                ...
                if ascmc is None:
                    raise ValueError(
                        "Swiss Ephemeris could not calculate Placidus houses (for Whole sign ASC)"
                    )
...
def calculate_chart(
    ...
    utc_dt = get_utc_datetime(
        year, month, day, hour, minute, input_is_ut, tz_offset, tz_name
    )
    ...
    for name, ident in loop_objects:
        ...
        second_in_sign = int(
            ((((lon_ % 30) - degree_in_sign) * 60) - minute_in_sign) * 60
        )
    ...
    cusp_signs_maps = {
        sys: _compute_cusp_signs(cusps_by_system[sys]) for sys in systems
    }
...
def _compute_cusp_signs(cusps_list):
    ...
    return {
        i + 1: _sign_from_degree(cusps_list[i]) for i in range(min(12, len(cusps_list)))
    }
...
def house_rulers_from_cusps(cusps_list, planetary_rulers, compute_cusp_signs_fn):
    ...
    """
    cusp_signs = compute_cusp_signs_fn(
        cusps_list
    )  # expects {1:'Aries', 2:'Taurus', ...}
    house_rulers = {}
...
def house_rulers_across_systems(
    cusps_by_system, planetary_rulers, compute_cusp_signs_fn
):
    """
...
_ASPECTS_MAJOR = {
    ...
    "Semi-sextile": {
        "angle": 30,
        "orb": 2,
    },  # minor, kept here for unified scan; we'll sort into minors later
...
def _applying_or_separating(
    lon1: float, speed1: float, lon2: float, speed2: float, target: float
) -> str:
    """
...
def annotate_reception(df: pd.DataFrame, edges_major: list[tuple]) -> pd.DataFrame:
    ...
    for a, b, meta in edges_major:
        asp = meta.get("aspect")
        if (
            asp in _RECEPTION_ASPECT_NAMES
