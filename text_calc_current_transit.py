...
if synastry_mode:
	test_chart_2 = st.radio(
		"Chart 2 (Outer)",
		["Custom", "Wildhorse", "Joylin", "Terra", "Jessica", "Current Transits"],
		horizontal=True,
		key="test_chart_2",
		label_visibility="collapsed"
	)
...
if synastry_mode and test_chart_2:
	...
	print(f" last_df_2 is None: {df_missing}")
...
def _refresh_chart_figure(chart_key=None):
	...
	if synastry_mode:
		...
		if df_inner is None or df_outer is None:
			...
			if df_inner is None:
				return
			st.write("Chart 2 failed to load.")
	...
	df = st.session_state.get(chart_key)
	pos = st.session_state.get(f"{chart_key}_chart_positions")

	if df is None or pos is None:
		return

	patterns = st.session_state.get(f"{chart_key}_patterns") or []
	shapes = st.session_state.get(f"{chart_key}_shapes") or []
	filaments = st.session_state.get(f"{chart_key}_filaments") or []
	combos = st.session_state.get(f"{chart_key}_combos") or {}
	singleton_map = st.session_state.get(f"{chart_key}_singleton_map") or {}
	major_edges_all = st.session_state.get(f"{chart_key}_major_edges_all") or []
	...
	edges_major = st.session_state.get(f"{chart_key}_edges_major") or []
	edges_minor = st.session_state.get(f"{chart_key}_edges_minor") or []
...
# Define chart_key based on the context
chart_key = "last_df" if not synastry_mode else "last_df_2"

def run_chart(lat, lon, tz_name, chart_key="last_df"):
	"""
	Build chart DF, aspects, dispositors, clusters, circuits/shapes—then render.
	Updates session state dynamically based on the chart_key.
	"""
	# --- Inputs from session ---
	...
	st.session_state[f"{chart_key}_dt_utc"] = new_chart_dt_utc
	...
	try:
		st.session_state[f"{chart_key}_sect"] = chart_sect_from_df(df)
		st.session_state[f"{chart_key}_sect_error"] = None
	except Exception as e:
		st.session_state[f"{chart_key}_sect"] = None
		st.session_state[f"{chart_key}_sect_error"] = str(e)

	clusters_rows = build_conjunction_clusters(df, edges_major)
	st.session_state[f"{chart_key}_conj_clusters_rows"] = clusters_rows
	...
	# Dynamically update session state for the specified chart
	st.session_state.update({
		f"{chart_key}": df,
		f"{chart_key}_aspect_df": aspect_df,
		f"{chart_key}_edges_major": edges_major,
		f"{chart_key}_edges_minor": edges_minor,
		f"{chart_key}_patterns": patterns,
		f"{chart_key}_shapes": shapes,
		f"{chart_key}_filaments": filaments,
		f"{chart_key}_singleton_map": singleton_map,
		f"{chart_key}_combos": combos,
		f"{chart_key}_chart_positions": pos_chart,
		f"{chart_key}_major_edges_all": major_edges_all
	})

	if synastry_mode:
		# Ensure both charts are available for biwheel rendering
		if "last_df" in st.session_state and "last_df_2" in st.session_state:
			if st.session_state["last_df"] is st.session_state["last_df_2"]:
				st.error("The same chart cannot be loaded into both wheels. Please select different charts.")
			else:
				fig = render_biwheel_chart(
					df_inner=st.session_state["last_df"],
					df_outer=st.session_state["last_df_2"],
					dark_mode=st.session_state.get("dark_mode", False),
					house_system=st.session_state.get("house_system", "placidus"),
					label_style=st.session_state.get("label_style", "glyph"),
				)
			...
			st.error("Both charts must be available for biwheel rendering.")
	else:
		_refresh_chart_figure(chart_key)
...
# Reinstate Current Transits functionality for Chart 2 in synastry mode
if synastry_mode:
	# Secondary chart selector (only visible in synastry mode)
	if test_chart_2 == "Current Transits":
		# Input for Current City
		current_city = st.text_input("Current City", key="current_city_transits")

		# Button to calculate Chart 2 with current transits
		if st.button("Calculate Chart 2"):
			if current_city:
				lat, lon, tz_name, formatted_address = _geocode_city_with_timezone(current_city)
				if lat and lon and tz_name:
					now = datetime.now(ZoneInfo(tz_name))
					st.session_state["city_2"] = formatted_address
					st.session_state["chart_dt_utc_2"] = now
					st.session_state["latitude_2"] = lat
					st.session_state["longitude_2"] = lon
					st.session_state["timezone_2"] = tz_name

					# Initialize singleton_{planet} keys
					for planet in PLANETS_PLUS.keys():
						st.session_state.setdefault(f"singleton_{planet}", False)

					# Call run_chart with Chart 2 data
					run_chart(lat, lon, tz_name, "last_df_2")
					st.success(f"Chart 2 calculated for {formatted_address} at {now}")
				else:
					st.error("Could not geocode the city. Please try again.")
			else:
				st.error("Please enter a city.")
	else:
		st.write("test_chart_2 not found")
...
if df_cached is not None:
	...
	# Add missing if synastry_mode block at line 1295
	if synastry_mode:
		# Ensure proper handling of synastry mode logic
		if "last_df" in st.session_state and "last_df_2" in st.session_state:
			if st.session_state["last_df"] is st.session_state["last_df_2"]:
				st.error("The same chart cannot be loaded into both wheels. Please select different charts.")
			else:
		...
		# Ensure the correct chart figure is rendered
		if chart_key == "last_df":
			st.session_state.update({
				"last_df_calc_lat": lat,
				"last_df_calc_lon": lon,
				"last_df_calc_tz": tz_name
			})
		elif chart_key == "last_df_2":
			st.session_state.update({
				"last_df_2_calc_lat": lat,
				"last_df_2_calc_lon": lon,
				"last_df_2_calc_tz": tz_name
			})


	# Ensure fig is a Matplotlib Figure
	fig = st.session_state.get("render_fig")
	if hasattr(fig, "fig"):  # Check if it's a RenderResult object
		fig = fig.fig  # Extract the Matplotlib Figure
