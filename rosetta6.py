...
# --- robust AC fetch (prefer Asc row, else 1st house cusp, else fallback) ---
def _safe_asc_deg(df):
	obj = df["Object"].astype("string")

	# 1) Direct Ascendant row, if present
	asc_mask = obj.str.match(r"^\s*(Ascendant|ASC|AC)\s*$", case=False, na=False)
	r = df[asc_mask]
	if not r.empty:
		for col in ("abs_deg", "Longitude"):
			if col in r.columns and r.iloc[0][col] not in (None, "", "nan"):
				try:
					return float(r.iloc[0][col]) % 360.0
				except Exception:
					pass  # fall through

	# 2) House 1 cusp variants
	cusp_mask = (
		obj.str.contains(r"\b(?:house\s*1|1\s*h)\s*cusp\b", case=False, regex=True, na=False)
		| obj.str.match(r"^\s*1\s*H\s*Cusp\s*$", case=False, na=False)
	)
	c = df[cusp_mask]
	if not c.empty:
		for col in ("abs_deg", "Longitude"):
			if col in c.columns and c.iloc[0][col] not in (None, "", "nan"):
				try:
					return float(c.iloc[0][col]) % 360.0
				except Exception:
					pass  # fall through

	# 3) Fallback to existing helper (last resort)
	try:
		return get_ascendant_degree(df) % 360.0
	except Exception:
		return 0.0
...
def render_chart_with_shapes(
	...
	# Sync with any DF recompute done above
	df  = st.session_state.get("df", df)     # if auto-heal ran, this is the fresh DF
	pos = st.session_state.get("pos", pos)   # keep positions aligned to that DF
	asc_deg = get_ascendant_degree(df)       # recompute AC from the current DF
...
def run_chart(lat, lon, tz_name, house_system):
	...
	if not is_valid_timezone(tz_name):
		st.error("Timezone could not be determined. Enter UTC offset manually.")
		offset_str = st.text_input(
			"UTC offset from UTC in hours (e.g., -5, 5.5)", key="manual_utc_offset"
		)
		try:
...
if st.session_state.get("chart_ready", False):
	...
	with st.expander("Interpretation Prompt"):
		...
		def strip_html_tags(text):
			# Preserve line breaks for the parser: turn block/line tags into \n
