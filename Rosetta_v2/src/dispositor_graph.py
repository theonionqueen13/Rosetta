...
def render_dispositor_section(st, df_cached) -> None:
	...
	if plot_data is not None:
		...
		if scope_data and scope_data.get("raw_links"):
			...
			with graph_col:
				# Debugging: Add print statements to inspect scope_data and header_info
				print("Debug: scope_data before calling plot_dispositor_graph:", scope_data)
				print("Debug: header_info before calling plot_dispositor_graph:", header_info)

				# Call plot_dispositor_graph
				try:
					disp_fig = plot_dispositor_graph(scope_data, planets_df=df_cached, header_info=header_info)
				except Exception as e:
					print("Error in plot_dispositor_graph:", e)
					raise
...
def position_tree_nodes(tree_edges, level_nodes, min_spacing, vertical_gap):
	...
	def position_subtree(node_id):
		...
		if node_id in visited:
			print(f"🔄 Node '{node_id}' already visited. Skipping to prevent cycles.")
			return 0  # Prevent infinite recursion due to cycles
		visited.add(node_id)

		level = next((lev for lev, ids in level_nodes.items() if node_id in ids), None)
		if level is None:
			print(f"⚠️ Warning: Node '{node_id}' not found in level_nodes.")
			return 0

		print(f"📍 Positioning node '{node_id}' at level {level}.")
		children = tree_edges.get(node_id, [])
		print(f"   Children of '{node_id}': {children}")
		...
		if not children:
			...
			print(f"   Leaf node '{node_id}' positioned at ({x}, {-level * vertical_gap}).")
			return x
		else:
			...
			print(f"   Internal node '{node_id}' positioned at ({x}, {-level * vertical_gap}).")
			return x

	# Start positioning from the root node
	root_id = next(iter(level_nodes.get(0, [])), None)
	if root_id:
		print(f"🌳 Starting positioning from root node '{root_id}'.")
		position_subtree(root_id)
	else:
		print("⚠️ No root node found in level_nodes.")
	...
	if unpositioned_nodes:
		print(f"⚠️ Warning: The following nodes were not positioned: {unpositioned_nodes}")
		for node in unpositioned_nodes:
			...
			next_x_by_level[level] = x + min_spacing
			print(f"   Fallback: Node '{node}' positioned at ({x}, {-level * vertical_gap}).")
...
def log_debug_info(phase, **kwargs):
	"""
	Log debugging information for a specific phase.

	Args:
		phase: The name of the phase being logged.
...
# Example usage in various phases:
# log_debug_info("Self-Ruling Trees", self_ruling_parents=self_ruling_parents, accounted_for=len(accounted_for))
# log_debug_info("Fallback Handling", remaining_nodes=len(remaining_nodes), roots=len(roots))

# Add calls to log_debug_info in all relevant functions and phases
...
def get_sign_aspect_name(p1_name, p2_name, planets_df):
	...
	try:
		...
		# Match names: 'South_Node' -> 'South Node'
		...
		n1 = ZODIAC_NUMBERS.get(s1, 0)
		n2 = ZODIAC_NUMBERS.get(s2, 0)
...
def plot_dispositor_graph(plot_data, planets_df, header_info=None):
	...
	print(f"🔮 Rulership loops detected: {sorted(rulership_loops)}")
	...
	for ax, tree_data in zip(axes, all_tree_data):
		...
		# Debugging: Print all edges being processed
		...
		for parent_id, child_id in edges:
			...
			print(f"Edge: {edge_key}, Meta: {meta}")  # Debugging: Print meta data for each edge
			aspect = meta.get('aspect')
			if aspect in _RECEPTION_ASPECTS:
				pair_to_aspect[edge_key] = aspect

		# Debugging: Print the final pair_to_aspect mapping
		print(f"pair_to_aspect mapping: {pair_to_aspect}")
		...
		png_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "pngs"))
		edges_major = st.session_state.get('edges_major', [])
		
		# DEBUGGING: Show graph diagnostic info in sidebar
		st.sidebar.header("Graph Diagnostic")
		st.sidebar.write(f"Sovereigns: {len(sovereigns)}")
		st.sidebar.write(f"Major Edges (Orbs) in State: {len(edges_major)}")
		
		if not edges_major:
			st.sidebar.warning("Warning: 'edges_major' is empty in session state.")
		# END DEBUG BLOCK

		# --- PHASE 4: Render Arrows and Aspect Glyphs ---
		edges_major = st.session_state.get('edges_major', [])
		...
		for parent_id, child_id in edges:
			...
			if start and end:
				...
				# 2. Extract base names for lookup (stripping the suffixes like _1_0)
				# We use .rsplit('_', 2)[0] to handle names like "South_Node_1_0" correctly
				p_name = parent_id.rsplit('_', 2)[0]
				c_name = child_id.rsplit('_', 2)[0]

				# 3. Find aspect metadata in session state
				# We check both directions (Parent-Child and Child-Parent) just in case
				aspect_meta = next(
					(meta for p, c, meta in edges_major 
					 if (p == p_name and c == c_name) or (p == c_name and c == p_name)), 
					None
				)

				if aspect_meta:
					aspect = aspect_meta.get("aspect")
					if aspect in _RECEPTION_ASPECTS:
						glyph = ASPECTS[aspect]["glyph"]
						
						# Calculate midpoint for glyph placement
						mid_x = (start[0] + end[0]) / 2
						mid_y = (start[1] + end[1]) / 2
						
						# Render the glyph with a small white background (bbox) 
						# so the arrow line doesn't cut through the symbol
						ax.text(
							mid_x, mid_y, glyph, 
							ha='center', va='center',
