...
# Extracted to src/profile_helpers.py — parameterised on a state dict so both
# Streamlit (st.session_state) and NiceGUI can share the same logic.
from src.profile_helpers import birth_data_from_chart as _birth_data_from_chart
from src.profile_helpers import apply_profile as _apply_profile_raw

def _apply_profile_to_session(prof_name, prof_data):
	"""Thin wrapper: delegates to src.profile_helpers.apply_profile with st.session_state."""
	return _apply_profile_raw(prof_name, prof_data, st.session_state)
