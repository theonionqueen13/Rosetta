...
def _detect_shapes_for_members(
            ...
            for pair_primary, pair_secondary in ((opp1, opp2), (opp2, opp1)):
                for a, d in (pair_primary, pair_primary[::-1]):
                    for b, e in (pair_secondary, pair_secondary[::-1]):
                        ...
                        if not all(
                            has_edge(x, y, asp) for (x, y), asp in sextile_specs
                        ):
                            continue
    ...
    for quad in combinations(R, 4):
        ...
        candidate_edges = sextile_specs + diag_specs
        add_once(
            "Mystic Rectangle", (a, b, c, d), candidate_edges, {"suppress": suppresses}
        )
...
def detect_shapes(pos, patterns, major_edges_all):
    ...
    for i in range(len(unnamed_info)):
        for j in range(i + 1, len(unnamed_info)):
            ...
            if not (
                aspect_match(q1, q2, "Quincunx")
                and aspect_match(q1, r1, "Square")
                and aspect_match(q2, r2, "Square")
                and aspect_match(q1, r2, "Trine")
                and aspect_match(q2, r1, "Trine")
            ):
                continue
...
def apply_suppression(shapes):
            ...
            for sub_members in sub_sets:
                for j, s_small in enumerate(shapes):
                    ...
                    if frozenset(s_small["members"]) == sub_members:
                        ...
                        if priority.get(s_big["type"], 0) >= priority.get(
                            s_small["type"], 0
                        ):
                            # Check keep rules
                            keep_hit = False
    ...
    return [
        ...
        if i not in suppressed or s["type"] == "Lightning Bolt"
    ]
