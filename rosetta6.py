...
if st.session_state.get("chart_ready", False):
    ...
    for obj, row in enhanced_objects_data.items():
        ...
        if h and h in cusp_signs:
            ...
            row["House Ruler"] = (
                list(rul) if isinstance(rul, (list, tuple)) else ([rul] if rul else [])
            )
    ...
    for obj in ordered_objects:
        ...
        dignity = _resolve_dignity(
            name_for_header, row.get("Sign", "")
        )  # uses your helper
        ...
        if station_flag:
            suffix_bits.append(
                "Stationing Direct"
            )  # or "Stationing" if you prefer shorter
        if rx_flag:
        ...
        if name_for_header in header_line:
            new_header_line = header_line.replace(
                name_for_header, name_for_header + header_suffix, 1
            )
        ...
        if dignity:
            ...
            sign_deg_line = (
                f"{row.get('Sign','').strip()} {row.get('Degree', '').strip()}"
            )
            ...
            if m:
                ...
                profile = profile.replace(
                    plain_sign_line, plain_sign_line + "<br/>" + dignity, 1
                )
            else:
