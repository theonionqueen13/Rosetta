...
def draw_degree_markers(ax, asc_deg, dark_mode):
    ...
    for deg in range(0, 360, 1):
        ...
        ax.plot([r, r], [circle_r, circle_r + 0.015], color=base_color, linewidth=0.5)

    # --- Ticks every 5°
    for deg in range(0, 360, 5):
        r = deg_to_rad(deg, asc_deg)
        ax.plot([r, r], [circle_r, circle_r + 0.03], color=base_color, linewidth=0.8)
    ...
    for idx in active_parents:
        if idx < len(patterns):
            ...
            if active_parents:
                ...
                draw_aspect_lines(
                    ax, pos, patterns, active_parents, asc_deg, edges=edges
                )
                ...
                for p_idx in active_parents:
                    ...
                    if len(active_parents) > 1:
                        ...
                        draw_minor_edges(
                            ax, pos, minor_edges, asc_deg, group_color=parent_color
                        )
                    else:
                ...
                for p1, p2, asp_name, pat1, pat2 in filaments:
                    ...
                    if (
                        "claimed_internal_minors" in locals()
                        and pair_key in claimed_internal_minors
                    ):
                        continue
...
def ask_gemini(
    prompt_text: str, model: str = "gemini-1.5-flash", temperature: float = 0.2
) -> str:
...
if st.session_state.get("chart_ready", False):
    ...
    with left_col:
        ...
        with st.expander("Click to Expand Instructions"):
            ...
            st.caption("View planet profiles on the left sidebar (» on mobile). ")
            st.caption(
    ...
    with colC:
        model = st.selectbox(
            "Model", ["gemini-1.5-flash", "gemini-1.5-pro"], index=0, key="gpt_model"
        )
    ...
    interp_text = st.session_state.get(
        "latest_interpretation", "_(Click **Send to Gemini** above to generate.)_"
    )
