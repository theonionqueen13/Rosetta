...
def _detect_shapes_for_members(pos, members, parent_idx, sid_start=0):
    ...
    for trio in combinations(members, 3):
        ...
        for (x, y, z) in permutations(trio):
            ...
            if status1 == status2 == status3 == "present":
                shapes.append({
                    "id": f"shape{sid}", "type": "T-Square",
                    "members": [x, y, z],
                    "edges": [((x, y), "Opposition"),
                              ((x, z), "Square"),
                              ((y, z), "Square")],
                    "parent": parent_idx
                })
    ...
    for quad in combinations(members, 4):
        for trio in combinations(quad, 3):
            ...
            tA, tB, tC = trio
            if (aspect_match(angle_delta(pos[tA], pos[tB]), "Trine")[0] == "present" and
                aspect_match(angle_delta(pos[tB], pos[tC]), "Trine")[0] == "present" and
                aspect_match(angle_delta(pos[tA], pos[tC]), "Trine")[0] == "present" and
                aspect_match(angle_delta(pos[apex], pos[tA]), "Opposition")[0] == "present"):
                shapes.append({
                    "id": f"shape{sid}", "type": "Kite",
