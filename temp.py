...
with col_left:
	with st.expander("📆 Enter Birth Data", expanded=True):
		...
		with st.form("birth_form", clear_on_submit=False):
			...
			# 1. Retrieve the calculated result object from the session state
render_result = st.session_state.get("render_result")

if render_result:
	# ⬇️ 2. USE DOT NOTATION TO ACCESS ATTRIBUTES ⬇️
	# Assuming render_result is an object with 'patterns', 'shapes', 'singleton_map' attributes
	try:
		patterns = render_result.patterns
		shapes = render_result.shapes
		singleton_map = render_result.singleton_map
	except AttributeError:
		# Fallback if the object exists but is somehow missing the attributes 
		# (shouldn't happen if the calculation succeeded, but safe programming)
		patterns = []
		shapes = []
		singleton_map = {}
else:
	# 3. Handle the initial run case (no chart calculated yet)
	patterns = []
	shapes = []
	singleton_map = {}
...
# 4. Call the UI function with the correctly populated data
...
# Store aspect toggles to session state so _refresh_chart_figure can access them
st.session_state["aspect_toggles"] = aspect_toggles
st.session_state["chart_mode"] = chart_mode # Added to explicitly store the mode returned by the UI
	...
	with col_2:
		...
		if chart_mode == "Standard Chart":
			...
			if transit_mode:
				...
				with tcol1:
					if st.form_submit_button("Previous", key="transit_prev"):
						st.session_state["transit_date_offset"] = st.session_state.get("transit_date_offset", 0) - 1
				with tcol2:
					if st.form_submit_button("Next", key="transit_next"):
						st.session_state["transit_date_offset"] = st.session_state.get("transit_date_offset", 0) + 1
