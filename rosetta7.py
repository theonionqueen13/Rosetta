...
def _canonical_shape_name(shape_dict: dict) -> str:
    ...
    candidates = [
        shape_dict.get("type"),
        shape_dict.get("kind"),
        shape_dict.get("shape"),
        shape_dict.get("shape_type"),
        shape_dict.get("label"),
        shape_dict.get("name"),
        shape_dict.get("parent"),
        shape_dict.get("parent_name"),
        shape_dict.get("title"),
        shape_dict.get("display"),
        shape_dict.get("display_name"),
    ]
...
st.caption(
    "Mobile users: click » at the top left to login, and to view planet profiles"
)
...
with st.sidebar:
    ...
    if auth_status is not True:
        ...
        if show_reset:
            ...
            if st.button("Email me a reset code", key="sb_btn_sendcode"):
                ...
                if not ok:
                    ...
                    if msg == "sent":
                        st.success(
                            "If that account exists, a code was sent. Check your email."
                        )
                    else:
        ...
        if st.session_state.get("sb_show_reset_step2"):
            ...
            npw2 = st.text_input(
                "Confirm new password", type="password", key="sb_reset_np2"
            )
...
if auth_status is True:
    ...
    ok, msg = ensure_user_row_linked(
        current_user_id, creds, display_name=(name or current_user_id)
    )
    if not ok:
        st.error(msg)
        st.stop()
...
# ============================
def clear_all_circuit_toggles():
    # Clear shape and pattern checkboxes immediately
    for k in list(st.session_state.keys()):
        if (
            k.startswith("toggle_pattern_")
            or k.startswith("shape_")
            or k.startswith("singleton_")
        ):
            st.session_state.pop(k, None)

    # Schedule Compass Rose to start unchecked next run
...
def _set_focus_selection(value, *, source: str, toggle_if_same: bool = False):
    ...
    if normalized == current:
        if (
            normalized is None
            and sentinel
            and st.session_state.get("focus_select") != sentinel
        ):
...
if (
    st.session_state.get("selected_placement")
    and st.session_state["focus_select"] != st.session_state["selected_placement"]
):
...
def format_planet_profile(row):
    ...
    header = (
        f"<div style='font-weight:bold; font-size:1.1em;'>{glyph} {canonical}</div>"
    )
    ...
    if meaning:
        html_parts.append(
            f"<div style='font-size:0.9em; margin-bottom:4px;'>{meaning}</div>"
        )
...
with col_mid:
    ...
    if st.button("Calculate Chart"):
        ...
        st.caption(
            "Instructions: Open the Circuit expanders to reveal their sub-shapes. Choose one to toggle on, scroll down to the Aspect Interpretation Prompt, and paste that into your ChatGPT. Joylin recommends beginning with whichever sub-shape includes your North Node."
        )
    ...
    # --- Load ---
        ...
        if saved_profiles:
            ...
            if (
                ...
                and "patterns" in st.session_state
            ):
                if st.button(
                    "💾 Save Circuit Names to Current Profile", key="save_names_only"
                ):
...
def _one_full_parent_selected(aspect_blocks):
    ...
    try:
        ...
        for b in aspect_blocks or []:
            ...
            if (
                b.get("is_parent") is True
                or kind in {"parent", "circuit"}
                or b.get("level") == 0
            ):
                parent_markers += 1
...
def ask_gpt(
    prompt_text: str, model: str = "gpt-4o-mini", temperature: float = 0.2
) -> str:
...
def _canonical_shape_name(shape_dict):
    ...
    raw = (
        shape_dict.get("type")
        or shape_dict.get("kind")
        or shape_dict.get("shape")
        or shape_dict.get("label")
        or shape_dict.get("name")
        or ""
    )
...
def _focus_core_from_df(df, focus_name: str):
    """Return list of objects that are conjunct with focus_name (cluster),
    using ALL objects present in df and your global Conjunction orb."""
    if not focus_name:
        return []

    # positions for ALL objects we actually have in this chart
    pos_all = dict(zip(df["Object"].astype(str), df["abs_deg"].astype(float)))
    if focus_name not in pos_all:
        return []  # gracefully handle if an object isn’t in this chart

    members = list(pos_all.keys())
    rep_pos, rep_map, rep_anchor = _cluster_conjunctions_for_detection(pos_all, members)
    rep = rep_anchor.get(focus_name, focus_name)
    cluster = rep_map.get(rep, [focus_name])
    # Keep original order by degree to be stable in UI
...
if st.session_state.get("chart_ready", False):
    ...
    for obj in ordered_objects:
        ...
        # --- Focus control + profile on one row (radio-look) ---
        btn_col, body_col = st.sidebar.columns([1, 9])  # tiny left col for the dot
        ...
        with body_col:
            html = profile.lstrip()
            title_line, rest = html.split("\n", 1) if "\n" in html else (html, "")

            # Render the title inline with the button
            st.markdown(title_line, unsafe_allow_html=True)

            # Render the remainder beneath the title
            if rest.strip():
    ...
    with st.expander("Interpretation Prompt"):
            ...
            if len(cluster) >= 2:
                label = ", ".join(
                    sorted(
                        cluster,
                        key=lambda x: (
                            ordered_objects.index(x) if x in ordered_objects else 999
                        ),
                    )
                )
                for m in cluster:
        ...
        _aspect_rx = re.compile(
            r"^(.*?)\s+(Opposition|Trine|Sextile|Square|Quincunx)\s+(.*)$",
            re.IGNORECASE,
        )
        ...
        if compass_on:
            ...
            if "South Node" in pos and "North Node" in pos:
                _bullets.append(
                    f"• {_label('South Node')} Opposition {_label('North Node')}"
                )

            # AC–DC axis
            if "Ascendant" in pos and "Descendant" in pos:
                _bullets.append(
                    f"• {_label('Ascendant')} Opposition {_label('Descendant')}"
                )
        ...
        if has_shapes_or_circuits:
            ...
            ordered_objects_prompt = list(ordered_objects) + [
                o for o in _compass_objs if o not in ordered_objects
            ]
            ...
            objs_for_profiles = sorted(
                enhanced_objects_data.keys(), key=lambda o: _order_index.get(o, 10_000)
            )
            ...
            objs_for_profiles = sorted(
                objs_for_profiles,
                key=lambda o: (
                    _compass_priority.get(o, 100),
                    _order_index.get(o, 10_000),
                    str(o),
                ),
            )
            ...
            for obj in objs_for_profiles:
                ...
                if not row:
                    continue
                if row:
                    no = ALIASES_MEANINGS.get(
                        obj, obj
                    )  # normalized name (e.g., "True Node" -> "North Node")

                profile = format_planet_profile(row)
                profile = strip_html_tags(profile)
                profile_text = profile

                # --- Prepend interpretation before the object name, with a trailing colon
                interp = (
                    OBJECT_INTERPRETATIONS.get(no)
                    or OBJECT_INTERPRETATIONS.get(obj)
                    or ""
                ).strip()
                ...
                if lines and name_for_edit:
                    ...
                    if pos2 != -1:
                        ...
                        if paren_suffix and (
                            end >= len(rest) or not rest[end:].lstrip().startswith("(")
                        ):
                ...
                try:
                    ...
                    sign_hdr_idx = next(
                        (
                            i
                            for i, t in enumerate(lines)
                            if t.lower() == "rulership by sign:"
                        ),
                        None,
                    )

                    house_line = (
                        lines[house_hdr_idx + 1]
                        if house_hdr_idx is not None and house_hdr_idx + 1 < len(lines)
                        else ""
                    )
                    sign_line = (
                        lines[sign_hdr_idx + 1]
                        if sign_hdr_idx is not None and sign_hdr_idx + 1 < len(lines)
                        else ""
                    )
            ...
            if shape_types_present:
                if len(shape_types_present) == 1:
                    ...
                    shape_list = (
                        ", ".join(shape_types_present[:-1])
                        + " and "
                        + shape_types_present[-1]
                        if len(shape_types_present) > 2
                        else " and ".join(shape_types_present)
                    )
                    interpretation_notes.append(
            ...
            present_categories = [
                cat for cat in ordered_cats if profiles_by_category[cat]
            ]
            _category_guides_todo = (
                present_categories  # defer appending to end of notes
            )
            ...
            if should_name_circuit:
                interpretation_notes.append(
                    "• Suggest a concise name (2-3 words) for the whole circuit."
                )
            ...
            sections = [
                base_intro,
                (
                    expanded_instructions if show_expanded else ""
                ),  # ← shows for shapes OR circuits
                (
                    compass_only_msg if compass_on else ""
                ),  # ← appended whenever Compass is ON
                (
                ...
                ),  # ← profiles included even Compass-only
            ...
            st.markdown(
                "_(Select at least 1 sub-shape **or** a circuit from the left to view a full circuit prompt, or toggle Compass Rose to view the Compass-only prompt.)_"
            )
    ...
    st.subheader(
        "Think of each of your planets (or clusters of planets, when conjunct), as a personified part of yourself. When you feel like you have parts of yourself either working together or in conflict, you do -- and this is the working map of those parts."
    )
    st.caption(
        "Keep checking back for more and more awesome interpretations as the app is developed! In the meantime, these ones are a great starting point for familiarizing yourself with your inner cast of characters. Synastry and transit readings coming soon, too!"
    )

    interp_text = st.session_state.get(
        "latest_interpretation", "_(Click **Send to GPT** above to generate.)_"
    )
    ...
    st.markdown(
        "_(Select at least 1 sub-shape or circuit, or the Compass Rose to view interpretation.)_"
    )
